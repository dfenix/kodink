...
//    val product = getProduct(state["product"] as ProductsState, it)
    CartProduct().apply {
//        id = product.id
//        title = product.title
//        price = product.price
...
fun getTotal(state: MutableMap<String, State>) = getAddedIds(state["cart"] as CartState).fold(0.0) {
    total, id -> total + getProduct(state["product"] as ProductsState, id).price * getQuantity(state["cart"] as CartState, id)
}
